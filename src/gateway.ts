...
const MESSAGE_DEDUP_WINDOW = 30000; // 30秒内不重复处理同一条消息
  ...
  const connect = async () => {
    ...
    try {
      ...
      const handleMessage = async (event: {
        ...
        // 消息去重检查
        if (isMessageProcessed(event.messageId)) {
          log?.debug(`[qqbot:${account.accountId}] Message ${event.messageId} already processed, skip`);
          return;
        }
        ...
        try {
          ...
          const dispatchPromise = pluginRuntime.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
            ...
            dispatcherOptions: {
              ...
              deliver: async (payload: { text?: string }) => {
                // 防止重复发送
                if (hasResponse) {
                  log?.debug(`[qqbot:${account.accountId}] Response already sent, skip duplicate`);
                  return;
                }
                hasResponse = true;
                if (timeoutId) {
              ...
              onError: async (err: unknown) => {
                // 防止重复错误处理
                if (hasResponse) {
                  log?.debug(`[qqbot:${account.accountId}] Error after response, skip`);
                  return;
                }
                hasResponse = true;
                if (timeoutId) {
                ...
                // 发送错误提示给用户
                if (errMsg.includes("401") || errMsg.includes("key") || errMsg.includes("auth") || errMsg.includes("token")) {
                  await sendErrorMessage("[ClawdBot] 大模型 API Key 可能无效，请检查配置");
                } else if (errMsg.includes("tool result") || errMsg.includes("tool id")) {
                  // 工具调用错误，不要重复发送
          ...
          const errMsg = String(err);
          // 避免重复发送错误消息
          if (errMsg.includes("tool result") || errMsg.includes("tool id") || errMsg.includes("not found")) {
            log?.error(`[qqbot:${account.accountId}] Tool call error (skip notification): ${errMsg.slice(0, 100)}`);
          } else {
